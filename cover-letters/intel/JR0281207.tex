...
\paragraph{} %
Thank you for considering my application for the Compiler Engineer role at Intel.
...
\paragraph{} %
I have also been developing my Around the World in 26 Languages project, where I experiment with a language or compiler for each letter of the alphabet [\href{\atwurl}{6}]. The languages are written in C++, and several target LLVM IR or MLIR dialects such as \emph{affine} and \emph{linalg}. I am also familiar with LLVM backend code generation, including SelectionDAG, TableGen, MIR, MachineScheduler, and GlobalISel. One entry in this project, Edinburgh, is an LLVM MIR scheduling playground that plugs an out-of-tree scheduler into LLVM's machine scheduler through \texttt{MachineSchedRegistry}, selects it through \texttt{-misched=edinburgh}, and compares the resulting MIR against LLVM's default scheduler [\href{\edinburghurl}{7}]. Through this project, I have continued developing practical compiler engineering experience in IR design, lowering pipelines, optimization passes, code generation, runtime interfaces, backend experimentation, and target-aware scheduling. Some of this work also runs in my private homelab environment, where I maintain infrastructure for compiler and systems experiments on local machines, VMs, and GPUs [\href{\homelaburl}{8}].



\paragraph{} %
For these reasons, I believe my background is a strong fit for compiler engineering work on backend optimization, code generation, and performance for Intel Architecture. I would be excited for the opportunity to contribute. Thank you again for your consideration.
...
\noindent \href{\homelaburl}{[8] Live Homelab Overview}
}
